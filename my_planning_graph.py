...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        for action in action_level:
            for literal_node in action.effnodes:
                if literal_node in literal_level:
                    for node in literal_level:
                    ...
                    literal_level.add(literal_node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if self.list_pair_have_common_elements(node_a1.action.effect_add, node_a2.action.effect_rem): return True
        if self.list_pair_have_common_elements(node_a1.action.effect_rem, node_a2.action.effect_add): return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        if self.list_pair_have_common_elements(node_a1.action.effect_add, node_a2.action.precond_neg): return True
        if self.list_pair_have_common_elements(node_a1.action.effect_rem, node_a2.action.precond_pos): return True
        if self.list_pair_have_common_elements(node_a1.action.precond_neg, node_a2.action.effect_add): return True
        if self.list_pair_have_common_elements(node_a1.action.precond_pos, node_a2.action.effect_rem): return True  
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            for i in range(len(self.s_levels)):
                literal_level = self.s_levels[i]
                literals = [literal_node.literal for literal_node in literal_level]
                if goal in literals:
                    level_sum += i
                    break
        return level_sum